...
- **Cache-busting:** every CSS/JS `<link>`/`<script>` uses `?v=N`. **After ANY css/js/data/image edit, bump N EVERYWHERE:**
  ```
  sed -i 's/?v=152/?v=153/g' index.html categorie.html contact.html marque.html styles.css app.js category.js
  ```
  (do NOT include `HANDOFF.md` — it rewrites this very line). **Currently at v=152.**
...
- **Caractéristiques** (added 2026-07-24, moved off the shelved PDP): `#catFeats`, re-rendered on every tab switch from `OMI_DATA.categoryContent[<slug>]`. `[hidden]` if a category has no `features`. **Structurally it IS the homepage trust strip** — same `.trust-inner`/`.trust-track`/`.trust-card`/`.trust-dots` markup, so 4-up row on desktop and **peek carousel + dots on mobile**.
  - **Styled after MOO's "Pourquoi MOO ?" band** (moo.com/fr/business-cards), values measured off the live page: **bg `#ECEFED`**, padding 64/72, h2 30px w400, lead 18px/1.5, card title 18px w400, card text 14px/1.5, 48px header→icons, **icons 72×72** (MOO's are also fills, `#00ac73`, at exactly 72px — the green just becomes OMI blue `#1E4DFF`). Ours are solid silhouettes where MOO's are thin outlined shapes, so they read slightly heavier; ~60px if that ever needs toning down. **Every one of these rules is scoped under `.cat-feats`** so the homepage strip keeps its own lighter scale (16/500 titles, 52px icons).
