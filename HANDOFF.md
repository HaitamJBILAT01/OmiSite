...
- **Cache-busting:** every CSS/JS `<link>`/`<script>` uses `?v=N`. **After ANY css/js/data/image edit, bump N EVERYWHERE:**
  ```
  sed -i 's/?v=154/?v=155/g' index.html categorie.html contact.html marque.html styles.css app.js category.js
  ```
  (do NOT include `HANDOFF.md` — it rewrites this very line). **Currently at v=154.**
