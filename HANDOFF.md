...
- **Cache-busting:** every CSS/JS `<link>`/`<script>` uses `?v=N`. **After ANY css/js/data edit, bump N in ALL pages + styles.css + product.js:** `sed -i 's/?v=145/?v=146/g' index.html categorie.html contact.html marque.html produit.html styles.css product.js` (do NOT include `HANDOFF.md` in that list — it rewrites this very line). **Currently at v=145.** (`product.js` is in the list because the **feature-icon SVG URL carries `?v=`** — SVGs have a 1-year cache in `.htaccess`, so a replaced `assets/<icon>.svg` won't show until the version bumps. The hero `<img>` + preload and the font `@font-face` URLs also carry `?v=`.) **Note:** JS-generated product-photo URLs (`assets/<image>.webp` in `cardHTML`/PDP) do NOT carry `?v=` — replacing a product photo needs a CDN purge + hard refresh, or add `?v=` there too.
...
- **KITCHEN HERO = per-category opt-in.** Set `heroKitchen:"kitchen.webp"` on a category in `categoryContent` (currently **only `sols-surfaces-vitres`**). product.js then adds `.pdp-has-kitchen` to `<main id="pdp">` and sets `#pdpKitchen`'s bg. CSS scoped under `.pdp-has-kitchen` neutralises the `.wrap` (`max-width:none;padding:0`) so the panel can reach the viewport edge. **Dettol's geometry, copied:**
  ...
  - **Curve is on the TOP-start corner** (`border-start-start-radius`), not the bottom — that's what Dettol does; flips in RTL.
  - **Product STANDS on the panel floor and breaks out above its top edge.** `.pdp-photo` is `position:absolute; bottom:<small>; height:calc(100% + <overflow>)` with `align-items:flex-end`. All sols photos are portrait 2:3, so `max-height` binds before `max-width:56%` and the overflow is reliable. **`.pdp-photo` must NOT get a `z-index`** — DOM order already paints it over `.pdp-kitchen`, and a z-index would create a stacking context that kills the blend below.
  - **Product photos still ship with a WHITE studio background** (not cutouts), so the img uses **`mix-blend-mode:multiply`** to knock that white out against the kitchen — that's what makes it read as part of the scene instead of a white box. `.pdp-media` has `isolation:isolate` to contain the blend to the panel. Delete the `mix-blend-mode` line to revert. Harmless once the photos are cut out.
