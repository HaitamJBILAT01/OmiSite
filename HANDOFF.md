...
- **Cache-busting:** every CSS/JS `<link>`/`<script>` uses `?v=N`. **After ANY css/js/data edit, bump N in ALL pages + styles.css:** `sed -i 's/?v=118/?v=119/g' index.html categorie.html contact.html marque.html styles.css`. **Currently at v=118.** (The hero `<img>` in index.html also carries `?v=` — the global sed catches it.)
- Fonts are **self-hosted** (`@font-face` in styles.css → `assets/fonts/general-sans-*.woff2`). Fontshare CDN was removed (unreliable on some networks). If font URLs ever need cache-busting, they use the same `?v=`.
