...
- **Cache-busting:** every CSS/JS `<link>`/`<script>` uses `?v=N`. **After ANY css/js/data/image edit, bump N EVERYWHERE:**
  ```
  sed -i 's/?v=156/?v=157/g' index.html categorie.html contact.html marque.html styles.css app.js category.js
  ```
  (do NOT include `HANDOFF.md` — it rewrites this very line). **Currently at v=156.**
...
## Tried and rejected
- **"Le bon geste" spotlight band** on the category page (built + reverted 2026-07-24, commits `0e46a20` → revert): Dettol's split panel — photo on one half, brand-blue panel on the other with one icon, a title and a per-category usage tip ("Un bouchon suffit", "Vingt secondes"…). Data lived in `categoryContent[<slug>].spotlight`. **User didn't like it; removed entirely.** `assets/test.webp` is the leftover placeholder photo — unused now, safe to delete. Recover the markup/CSS/copy from `0e46a20` if it's ever wanted again.
