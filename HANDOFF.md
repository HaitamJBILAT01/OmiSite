...
- **Cache-busting:** every CSS/JS `<link>`/`<script>` uses `?v=N`. **After ANY css/js/data/image edit, bump N EVERYWHERE:**
  ```
  sed -i 's/?v=155/?v=156/g' index.html categorie.html contact.html marque.html styles.css app.js category.js
  ```
  (do NOT include `HANDOFF.md` — it rewrites this very line). **Currently at v=155.**
...
Loads `data.js`, `app.js`, `category.js`. Order: navbar → **cat-hero** (shared `bannerCAT.webp`) → **cat-switch** (underline tabs: **the 6 categories in data order**, Maxi Plus last, NO "Tous") → **cat-range** (products) → **cat-spot (Le bon geste)** → **cat-feats (Caractéristiques)** → footer. No `?cat=` falls back to the FIRST category (sols) — it used to be the virtual Maxi Plus tab.
...
- **Spotlight / "Le bon geste"** (`#catSpot`, added 2026-07-24): Dettol's split band — **photo on one half, brand-blue gradient panel on the other** with ONE icon + title + a short usage tip. Data = `categoryContent[<slug>].spotlight` `{photo, icon, title{fr,ar}, text{fr,ar}}`; `[hidden]` if a category has none.
  - **Content is a practical usage tip, not a benefit** — that's what keeps it from duplicating Caractéristiques directly below ("Un bouchon suffit", "Jamais avec un autre produit"…). The Javel one is a real safety point: bleach + descaler/other cleaner releases dangerous fumes.
  - **No product photo and no title chip**, unlike Dettol's original: the PDP is shelved, and nothing else on the site uses a chip — the panel colour carries the emphasis instead.
  - **The photo is a CSS `background-image`, not an `<img>`** — a missing or renamed file degrades to an empty tinted half rather than a broken-image icon. **All 6 currently point at the same `test.webp` placeholder**; give each category its own photo by editing `spotlight.photo`.
  - The band has **no outer padding on purpose** — `.cat-range` ends with `var(--sec)` and `.cat-feats` opens with its own top padding, so the neighbours supply the rhythm (same pattern as `.mx` on the homepage).
  - `iconMarkup(icon, i, cls)` in category.js is now shared by both sections: `cls` picks the skin — `trust-icon` (blue on white) or `cat-spot-ic` (white on the blue panel). 3 of the 6 spotlight icons are still on the line-art fallback (`grease-cut`, `soft-drop`, `absorbent`).
- **Caractéristiques** (added 2026-07-24, moved off the shelved PDP): `#catFeats`, re-rendered on every tab switch from `OMI_DATA.categoryContent[<slug>]`. `[hidden]` if a category has no `features`. **Structurally it IS the homepage trust strip** — same `.trust-inner`/`.trust-track`/`.trust-card`/`.trust-dots` markup, so 4-up row on desktop and **peek carousel + dots on mobile**.
  ...
  - Page order today: white product grid → white Caractéristiques → dark footer.
