...
Layout mirrors Dettol: full-bleed 2-col hero (`.pdp-hero`) — **left** = breadcrumb, H1, short desc, pill selectors, aligned to the site content margin with top space under the navbar; **right** = a panel that **bleeds to the top (flush under the fixed navbar) and the right viewport edge**, with a **big curved bottom-left corner** (`.pdp-kitchen` `border-end-start-radius`, flips in RTL) — a blurred **kitchen background** with the **product bottle large on top** (`.pdp-photo img` max-height 90%, drop-shadow). On mobile (<900px) the panel + bottle go on top, copy below. Minimal: **no eyebrow, no summary line, no retailer/Amazon buttons.**
- **Kitchen photo = per-category `heroPhoto` in `categoryContent`** (all `""` for now → a dashed "Photo cuisine — à ajouter" placeholder via `renderKitchen()`; set a filename in `./assets/` to use a real image).
...
- **Selectors** come from the product's `axes`, all rendered as **plain text pills — no colour dots** (the `swatch`/`style` fields in data.js are legacy and unused). Axis groups sit side by side. A product with several variants but NO axes (e.g. `liquide-vaisselle`) gets one synthetic **"Variante"** row built from each variant's `sub`. Single-variant products get no selector.
