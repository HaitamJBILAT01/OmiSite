...
(function () {
  ...
  function photoHTML(variant) {
    ...
    return `<img src="assets/${file}?v=149" alt="${alt}" fetchpriority="high" decoding="async">`;
  }
  ...
  if (CONTENT.heroKitchen) {
    ...
    if (k) k.style.backgroundImage = `url('assets/${CONTENT.heroKitchen}?v=149')`;
  }
  ...
  function featIcon(icon) {
    ...
    return `<span class="pfeat-ic" style="--ic:url('assets/${icon}.svg?v=149')" aria-hidden="true"></span>`;
  }
  function photoBox(file, hint) {
    if (file) return `<img src="assets/${file}?v=149" alt="" loading="lazy" decoding="async">`;
  ...
  function renderExtraSections() {
    ...
    if (CONTENT.callout) {
      ...
      const shot = v ? `<img src="assets/${product.photo || v.image}?v=149" alt="${(v && v.alt) || product.name.fr}" loading="lazy" decoding="async">` : "";
    ...
    if (G.didYouKnow) {
      ...
      const bg = d.photo ? ` style="background-image:url('assets/${d.photo}?v=149')"` : "";
